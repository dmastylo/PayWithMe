module EventUsersHelper
  def event_user_pay_button(event_user, payment_method)
    ...
    link_to "Pay <span class=\"items-total\">#{humanized_money_with_symbol event_user.event.split_amount}</span> With #{payment_options}", path, class: "btn btn-primary btn-block push-bottom"
  end
end