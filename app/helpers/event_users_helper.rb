module EventUsersHelper
  def event_user_pay_button(event_user)
    link_to "Pay <span class=\"pay-total\">#{humanized_money_with_symbol event_user.payment.amount}</span> With Credit or Debit Card".html_safe, pay_payment_path(@event_user.payment), class: "btn btn-primary btn-block push-bottom"
  end

  # def event_user_pay_fundraiser_button(event_user)
